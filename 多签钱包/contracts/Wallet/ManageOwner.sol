...
import {SelfAuthorize} from "./SelfAuthorize.sol";

abstract contract ManageOwner is SelfAuthorize{
    ...
    // 查看下一个节点
    function getAdjacent(address owner, bool _direction)public view returns (bool, address) {
        if (!ownerExist(owner)) {
            return (false, _NULL);
        } else {
